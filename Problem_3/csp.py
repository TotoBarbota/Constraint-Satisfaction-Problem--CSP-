...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints):
        ...
        self.nassigns = 0
        # Initialize constraint weights for dom/wdeg
...
def dom_wdeg(assignment, csp):
    """Return a variable with minimum ratio of domain size to weighted degree.
    The weighted degree of a variable is the sum of the weights of the constraints
    that include the variable and are not satisfied by the current assignment."""
    unassigned = [v for v in csp.variables if v not in assignment]
    if not unassigned:
        return None
    
    def weighted_degree(var):
        # Calculate sum of weights of constraints involving var that are not satisfied
        weight_sum = 0
        for neighbor in csp.neighbors[var]:
            if neighbor not in assignment:  # Only count constraints with unassigned variables
                weight_sum += csp.constraint_weights[(var, neighbor)]
        return weight_sum if weight_sum > 0 else 1  # Avoid division by zero
    
    return argmin_random_tie(
        unassigned,
        key=lambda var: len(csp.curr_domains[var] or csp.domains[var]) / weighted_degree(var))
    ...
    def backtrack(assignment):
        ...
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                ...
                # Increment weights of violated constraints
                for neighbor in csp.neighbors[var]:
                    if neighbor not in assignment and not csp.constraints(var, value, neighbor, value):
